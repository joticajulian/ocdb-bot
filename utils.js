...
var HOURS = 60 * 60;
 ...
 var rewardFundLoaded = false;
 var steemPriceLoaded = false;
...
//config = JSON.parse(fs.readFileSync("config.json"));
 ...
 function updateSteemVariables() {
     ...
     steem.api.getCurrentMedianHistoryPrice(function (e, t) {
       if(t && !e) {
         steemPrice = parseFloat(t.base.replace(" HBD", "")) / parseFloat(t.quote.replace(" HIVE", ""));
         steemPriceLoaded = true;
       } else {
     ...
     if(rewardFundLoaded && steemPriceLoaded && globalPropertiesLoaded){
       var globalPropertiesSteem = {
         vote_power_reserve_rate: votePowerReserveRate,
         total_vesting_fund_steem: totalVestingFund,
         total_vesting_shares: totalVestingShares,
         steem_per_mvests: steem_per_mvests,
         steem_price: steemPrice,
         reward_balance: rewardBalance,
         recent_claims: recentClaims
       }
       firebase.database().ref(config.account+'/globalPropertiesSteem').set(globalPropertiesSteem);       
     }
